...
class DspApiTests(unittest.TestCase):
    ...
    def test_normalize_dsp_report_rows_applies_original_distributor_and_ad_rules(self) -> None:
        ...
        self.assertEqual(rows[1]["最終經銷商"], "HB串接")
        self.assertEqual(rows[1]["規則命中_經銷商"], "hb_vendor")
        self.assertEqual(rows[1]["最終來源_經銷商"], "rule")
        self.assertEqual(rows[1]["最終廣告形式"], "一般廣告")
        self.assertEqual(rows[1]["規則命中_廣告形式"], "rule:default")

        self.assertEqual(rows[2]["最終經銷商"], "外部經銷商")
        self.assertEqual(rows[2]["規則命中_經銷商"], "external_distributor")
        self.assertEqual(rows[2]["最終來源_經銷商"], "rule")
        self.assertEqual(rows[2]["最終廣告形式"], "影音摩天")
        self.assertEqual(rows[2]["規則命中_廣告形式"], "rule:video_16_9_cpm_lt40")
        ...
        self.assertEqual(rows[3]["最終廣告形式"], "一般廣告")
        self.assertEqual(rows[3]["規則命中_廣告形式"], "rule:canonical")
