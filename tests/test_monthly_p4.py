...
class MonthlyP4Tests(unittest.TestCase):
    ...
    def test_monthly_p4_snapshot_seeds_targets_and_uses_manual_inputs(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            ...
            self.assertEqual(float(april_after["actuals"]["external_total"]), 100.0)
            self.assertEqual(float(april_after["actuals"]["mf_marketing"]), 141000.0)
            self.assertEqual(float(april_after["computed"]["mf_marketing"]), 0.0)
    ...
    def test_monthly_charts_do_not_use_runtime_p4_before_close(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            root = Path(td)
            repo = self._setup_project(root)
            svc = CanonicalService(repo)
            ...
            self.assertEqual(april["p4MfActual"], 0.0)
            self.assertEqual(april["mediaCostInvestment"], 0.0)
            self.assertFalse(april["p4Closed"])
            self.assertEqual(april["p4InvestmentSource"], "unclosed")
